...
import { IRDiagram, IREdge, IRNode, IRStatement, IRGroup, IRNote, IRActivation, IRMember, IRReturn, IRAutoactivate, IRMainframe, IRContainer } from "../ir/types";
...
export class SequenceAstVisitor extends BaseVisitor {
    ...
    statement(ctx: any): IRStatement {
        ...
        if (ctx.noteDeclaration) {
            return this.visit(ctx.noteDeclaration[0]);
        }
        if (ctx.refDeclaration) {
            return this.visit(ctx.refDeclaration[0]);
        }
        if (ctx.shortFormDeclaration) {
            return this.visit(ctx.shortFormDeclaration[0]);
        }
        if (ctx.groupingBlock) {
            return this.visit(ctx.groupingBlock[0]);
        }
        if (ctx.activationDeclaration) {
            return this.visit(ctx.activationDeclaration[0]);
        }
        if (ctx.autoactivateStatement) {
            return this.visit(ctx.autoactivateStatement[0]);
        }
        if (ctx.returnStatement) {
            return this.visit(ctx.returnStatement[0]);
        }
        if (ctx.createStatement) {
            return this.visit(ctx.createStatement[0]);
        }
        if (ctx.containerBlock) {
            return this.visit(ctx.containerBlock[0]);
        }
        if (ctx.jsonBlock) {
            return this.visit(ctx.jsonBlock[0]);
        }
        if (ctx.togetherBlock) {
            return this.visit(ctx.togetherBlock[0]);
        }
        if (ctx.mainframeStatement) {
            return this.visit(ctx.mainframeStatement[0]);
        }
        if (ctx.anchor) {
            return this.visit(ctx.anchor[0]);
        }
        if (ctx.ignoredStatement) {
            return this.visit(ctx.ignoredStatement[0]);
        }
    ...
    jsonBlock(ctx: any): IRContainer {
        ...
        return {
            ...
            keyword: "json",
            name,
            statements: [], // JSON content can be processed later or kept as raw tokens
            offset: {
                start: ctx.Json[0].startOffset,
                end: ctx.RBrace[0].endOffset
            }
        };
    }
    ...
    togetherBlock(ctx: any): IRContainer {
        ...
        return {
            ...
            statements,
            offset: {
                start: ctx.Together[0].startOffset,
                end: ctx.RBrace[0].endOffset
            }
        };
    }
    ...
    colorValue(ctx: any): string {
        ...
        if (ctx.Visibility && ctx.Identifier) {
            return ctx.Visibility[0].image + ctx.Identifier[0].image;
        }
        return "";
    }
    ...
    shortFormDeclaration(ctx: any): IRNode {
        ...
        if (nameToken.tokenType && nameToken.tokenType.name === "StringLiteral") {
            origName = this.unquoteString(name);
        }
        ...
        if (ctx.Colon) {
            ...
            shape = "component";
        } else if (ctx.LParen && ctx.RParen && ctx.name[0].startOffset > ctx.RParen[0].startOffset) {
            // () interface
            shape = "interface";
        } else if (ctx.LParen) {
            shape = "usecase";
        ...
        if (ctx.stereotype) {
            stereotype = this.visit(ctx.stereotype[0]);
        }
        ...
        const startToken = ctx.Colon ? ctx.Colon[0] : 
                           ctx.LBracket ? ctx.LBracket[0] : 
                           ctx.LParen ? ctx.LParen[0] : nameToken;
        
        let endToken = ctx.Colon && ctx.Colon.length > 1 ? ctx.Colon[1] : 
                       (ctx.RBracket ? ctx.RBracket[0] :
                       (ctx.RParen ? ctx.RParen[0] : nameToken));
    ...
    participantDeclaration(ctx: any): IRNode {
        const shape = ctx.Participant ? "participant" : 
                      ctx.Actor ? "actor" : 
                      ...
                      ctx.Card ? "card" :
                      ctx.FileKeyword ? "file" :
                      ctx.Stack ? "stack" :
                      ctx.Hexagon ? "hexagon" :
                      ctx.Person ? "person" :
                      ctx.Process ? "process" :
                      ctx.Agent ? "agent" :
                      ctx.LabelKeyword ? "label" :
                      ctx.Usecase ? "usecase" :
                      ...
                      ctx.Frame ? "frame" :
                      ctx.Folder ? "folder" :
                      ctx.Package ? "package" : "participant";
        ...
        const startToken = ctx.Participant ? ctx.Participant[0] : 
                           ctx.Actor ? ctx.Actor[0] : 
                           ctx.Boundary ? ctx.Boundary[0] :
                           ctx.Control ? ctx.Control[0] :
                           ctx.Entity ? ctx.Entity[0] :
                           ctx.Database ? ctx.Database[0] :
                           ctx.Collections ? ctx.Collections[0] :
                           ctx.Queue ? ctx.Queue[0] :
                           ctx.Artifact ? ctx.Artifact[0] :
                           ctx.Storage ? ctx.Storage[0] :
                           ctx.Rectangle ? ctx.Rectangle[0] :
                           ctx.Card ? ctx.Card[0] :
                           ctx.FileKeyword ? ctx.FileKeyword[0] :
                           ctx.Stack ? ctx.Stack[0] :
                           ctx.Hexagon ? ctx.Hexagon[0] :
                           ctx.Person ? ctx.Person[0] :
                           ctx.Process ? ctx.Process[0] :
                           ctx.Agent ? ctx.Agent[0] :
                           ctx.LabelKeyword ? ctx.LabelKeyword[0] :
                           ctx.Usecase ? ctx.Usecase[0] :
                           ctx.Component ? ctx.Component[0] :
                           ctx.Action ? ctx.Action[0] :
                           ctx.Port ? ctx.Port[0] :
                           ctx.PortIn ? ctx.PortIn[0] :
                           ctx.PortOut ? ctx.PortOut[0] :
                           ctx.NodeKeyword ? ctx.NodeKeyword[0] :
                           ctx.Circle ? ctx.Circle[0] :
                           ctx.Interface ? ctx.Interface[0] :
                           ctx.Cloud ? ctx.Cloud[0] :
                           ctx.Frame ? ctx.Frame[0] :
                           ctx.Folder ? ctx.Folder[0] :
                           ctx.Package ? ctx.Package[0] : nameToken;
    ...
    multilineLabel(ctx: any): IToken {
        return ctx.RBracket[0];
    }
    ...
    name(ctx: any): IToken {
        ...
        if (ctx.part) {
            ...
            return {
                ...(ctx.leadingSep ? ctx.leadingSep[0] : parts[0]),
                image,
                endOffset
            };
        }
    ...
    classDeclaration(ctx: any): IRNode {
        ...
        const shape = ctx.Class ? "class" : 
                      ...
                      ctx.Record ? "record" :
                      ctx.Stereotype ? "stereotype" :
                      ctx.Dataclass ? "dataclass" :
                      ctx.ObjectKeyword ? "object" :
                      ctx.LParen ? "circle" :
                      ctx.DiamondShort ? "diamond" : "class";
                      
        const nameToken = this.visit(ctx.name[0]);
        let name = nameToken.image;
        if (nameToken.tokenType && nameToken.tokenType.name === "StringLiteral") {
            name = this.unquoteString(name);
        }
        ...
        if (ctx.classMember) {
            ctx.classMember.forEach((cm: any) => {
                members.push(this.visit(cm));
            });
        }
        ...
        if (ctx.parents) {
            ctx.parents.forEach((p: any) => {
                parents.push(this.visit(p).image);
            });
        }

        const startToken = ctx.Visibility ? ctx.Visibility[0] :
                           (ctx.Class ? ctx.Class[0] : 
                           ctx.Interface ? ctx.Interface[0] :
                           ctx.Enum ? ctx.Enum[0] :
                           ctx.Struct ? ctx.Struct[0] :
                           ctx.Annotation ? ctx.Annotation[0] :
                           ctx.Abstract ? ctx.Abstract[0] :
                           ctx.Circle ? ctx.Circle[0] :
                           ctx.Diamond ? ctx.Diamond[0] :
                           ctx.Exception ? ctx.Exception[0] :
                           ctx.Metaclass ? ctx.Metaclass[0] :
                           ctx.Protocol ? ctx.Protocol[0] :
                           ctx.Record ? ctx.Record[0] :
                           ctx.Stereotype ? ctx.Stereotype[0] :
                           ctx.Dataclass ? ctx.Dataclass[0] :
                           ctx.ObjectKeyword ? ctx.ObjectKeyword[0] :
                           ctx.LParen ? ctx.LParen[0] :
                           ctx.DiamondShort ? ctx.DiamondShort[0] : nameToken);
    ...
    classMember(ctx: any): IRMember {
        ...
        if (ctx.tokens) {
            ...
            if (memberLabelCtx.anyToken) {
                allTokens = memberLabelCtx.anyToken.map((at: any) => this.visit(at));
            }
        }
        
        // Find the last colon at paren depth 0
        ...
        for (let i = 0; i < allTokens.length; i++) {
            ...
            else if (typeName === "Colon" && parenDepth === 0) {
                typeSeparatorIndex = i;
            }
        }
        ...
        // Handle Color as visibility if it was at the beginning of name
    ...
    connectionDeclaration(ctx: any): IREdge {
        ...
        if (ctx.from) {
            fromData = this.visit(ctx.from[0]);
        } else {
            fromData = {
                name: "[",
                offset: { start: ctx.arrow[0].startOffset, end: ctx.arrow[0].startOffset }
            };
        }

        if (ctx.to) {
            toData = this.visit(ctx.to[0]);
        } else {
            toData = {
                name: "]",
                offset: { start: ctx.arrow[0].endOffset, end: ctx.arrow[0].endOffset }
            };
        }
        ...
        let color: string | undefined = undefined;
        if (ctx.colorValue) {
            color = this.visit(ctx.colorValue[0]);
        }
        if (ctx.colorValue1) {
            color = this.visit(ctx.colorValue1[0]);
        }
        if (ctx.colorValue2) {
            color = this.visit(ctx.colorValue2[0]);
        }

        let isCreation = false;
        if (ctx.Star && ctx.Star.length >= 2) {
            isCreation = true;
        }

        let isDeletion = false;
        if (ctx.Exclamation && ctx.Exclamation.length >= 2) {
            isDeletion = true;
        }
    ...
    payload(ctx: any): { text: string, endOffset: number } {
        if (ctx.StringLiteral) {
            ...
            return {
                text: this.unquoteString(token.image),
    ...
    endpoint(ctx: any) {
        ...
        if (ctx.label && ctx.name) {
            ...
            if (ctx.parts) {
                ctx.parts.forEach((p: any) => {
                    name += ", " + this.visit(p).image;
                });
            }
            name += ")";
    ...
    groupingBlock(ctx: any): IRGroup {
        const startToken = ctx.Alt ? ctx.Alt[0] : 
                           ctx.Opt ? ctx.Opt[0] : 
                           ctx.Loop ? ctx.Loop[0] : 
                           ctx.Par ? ctx.Par[0] : 
                           ctx.Group ? ctx.Group[0] : 
                           ctx.Partition ? ctx.Partition[0] : ctx.Box[0];
        const keyword = ctx.Alt ? "alt" : 
                        ctx.Opt ? "opt" : 
                        ctx.Loop ? "loop" : 
                        ctx.Par ? "par" : 
                        ctx.Group ? "group" : 
                        ctx.Partition ? "partition" : "box";
        ...
        sections.push({
            label,
            statements: ctx.statement ? ctx.statement.map((s: any) => this.visit(s)) : [],
        });
        ...
        if (ctx.Box && ctx.Box.length > 1) {
            endOffset = ctx.Box[1].endOffset;
        }
    ...
    noteDeclaration(ctx: any): IRNote {
        ...
        if (ctx.target) {
            ctx.target.forEach((t: any) => targets.push(this.visit(t).image));
        }
        ...
        let text = "";
        let endOffset = color ? startToken.endOffset : startToken.endOffset; // simplified for now
        ...
        if (ctx.payload) {
            ...
            if (ctx.alias) {
               const aliasToken = this.visit(ctx.alias[0]);
               endOffset = aliasToken.endOffset;
            }
        } else if (ctx.alias) {
            const aliasToken = this.visit(ctx.alias[0]);
            endOffset = aliasToken.endOffset;
        } else if (ctx.target) {
            const lastTarget = this.visit(ctx.target[ctx.target.length - 1]);
            endOffset = lastTarget.endOffset;
        }
    ...
    activationDeclaration(ctx: any): IRActivation {
        ...
        return {
            ...
            offset: {
                start: startToken.startOffset,
                end: color ? startToken.endOffset : ctx.Identifier[0].endOffset,
            }
        };
    }
    ...
    autoactivateStatement(ctx: any): IRAutoactivate {
        ...
        return {
            ...
            offset: {
                start: startToken.startOffset,
                end: (ctx.On || ctx.Off)[0].endOffset,
            }
        };
    }
    ...
    createStatement(ctx: any): IRNode {
        ...
        return {
            ...
            shape: "participant",
            name: nameToken.image,
            origName: nameToken.image,
            isCreation: true,
            offset: {
                start: ctx.Create[0].startOffset,
    ...
    mainframeStatement(ctx: any): IRMainframe {
        ...
        return {
            ...
            label,
            offset: {
                start: ctx.Mainframe[0].startOffset,
                end: ctx.label[0].endOffset,
            }
        };
    }
    ...
    containerBlock(ctx: any): IRContainer {
        const keywordToken = ctx.Package ? ctx.Package[0] :
                             ctx.Namespace ? ctx.Namespace[0] :
                             ctx.Folder ? ctx.Folder[0] :
                             ctx.Cloud ? ctx.Cloud[0] :
                             ctx.Frame ? ctx.Frame[0] :
                             ctx.Rect ? ctx.Rect[0] :
        ...
        if (ctx.name) {
            const nameToken = this.visit(ctx.name[0]);
            name = nameToken.image;
            if (nameToken.tokenType && nameToken.tokenType.name === "StringLiteral" && name !== undefined) {
                name = this.unquoteString(name);
            }
        }
        ...
        let endOffset = 0;
        if (ctx.RBrace) {
            endOffset = ctx.RBrace[0].endOffset;
        } else if (ctx.multilineLabel) {
            endOffset = this.visit(ctx.multilineLabel[0]).endOffset;
        }
        ...
        return {
            ...
            statements,
            offset: {
                start: keywordToken ? keywordToken.startOffset : 0,
                end: endOffset
            }
        };
    }
    ...
    private processUnquotedLabel(tokens: IToken[]): string {
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            if (current.tokenType && current.tokenType.name === "StringLiteral") {
                image = this.unquoteString(image);
            }
            result += image;
            
            if (i < tokens.length - 1) {
                const next = tokens[i + 1];
                if (next.startOffset > (current.endOffset || 0) + 1 && current.tokenType?.name !== "Newline" && next.tokenType?.name !== "Newline") {
                    result += " ";
                }
            }
        }
